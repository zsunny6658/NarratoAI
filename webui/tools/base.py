...
# 保留旧的导入以确保向后兼容
from app.utils import gemini_analyzer, qwenvl_analyzer, siliconflow_analyzer
...
def create_vision_analyzer(provider, api_key, model, base_url):
    ...
    if provider == 'siliconflow':
        from app.utils.siliconflow_analyzer import SiliconFlowAnalyzer
        return SiliconFlowAnalyzer(model_name=model, api_key=api_key, base_url=base_url)
    else:
        # 只传入必要的参数
